...
class RawParallelDatasetLoader:
    ...
    def load(self,
        ...
        for sources, target, src_graph in parallel_graph_iter(source_iterables, target_iterable, graph_iterable):
            source_len = len(sources[0])
            target_len = len(target)

            # For each instance we find its bucket based on its length.
...
def get_training_data_iters(sources: List[str],
    ...
    sources_sentences, target_sentences, graphs = create_graph_readers(sources, target, source_graphs,
                                                                       source_vocabs, target_vocab, edge_vocab)
...
class ParallelDataSet(Sized):
    ...
    def save(self, fname: str):
        ...
        """
        mx.nd.save(fname, self.source + self.target + self.label + self.src_graphs + self.src_positions)
        ...
        for bucket_idx in range(len(self)):
            ...
            if num_samples % bucket_batch_size != 0:
                if fill_up == 'replicate':
                    ...
                    graph[bucket_idx] = mx.nd.concat(bucket_graph, bucket_graph.take(random_indices), dim=0)
                    position[bucket_idx] = mx.nd.concat(bucket_position, bucket_position.take(random_indices), dim=0)
                else:
