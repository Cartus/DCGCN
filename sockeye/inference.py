...
class InferenceModel(model.SockeyeModel):
    ...
    def run_encoder(self,
                    ...
                    source_positions: mx.nd.NDArray) -> 'ModelState':
        """
...
class TranslatorInput:
    ...
    __slots__ = ('sentence_id', 'tokens', 'graph', 'factors', 'constraints', 'avoid_list')
    ...
    def __str__(self):
        return 'TranslatorInput(%s, %s, %s, factors=%s, constraints=%s, avoid=%s)' \
            % (self.sentence_id, self.tokens, self.graph, self.factors, self.constraints, self.avoid_list)
...
class Translator:
    ...
    def _get_inference_input(self,
        ...
        return source, bucket_key, new_graph, positions, raw_constraints, raw_avoid_list, mx.nd.array(max_output_lengths, ctx=self.context, dtype='int32')
    ...
    def _translate_nd(self,
        ...
        return self._get_best_from_beam(*self._beam_search(source, source_length, source_graph, source_positions, raw_constraints, raw_avoid_list, max_output_lengths))

    def _encode(self, sources: mx.nd.NDArray,
                source_length: int,
                source_graph: mx.nd.NDArray,
        ...
        """
        return [model.run_encoder(sources, source_length, source_graph, source_positions) for model in self.models]
